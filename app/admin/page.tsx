...
export default function AdminPage() {
  ...
  const [generalEmailTemplate, setGeneralEmailTemplate] = useState("roots")
  const [generalEmailAttachments, setGeneralEmailAttachments] = useState<File[]>([])

  // Add a new state variable for sender email after the existing general email state variables:
  const [generalSenderEmail, setGeneralSenderEmail] = useState("noreply@maubentech.com")
  ...
  const handleGeneralEmailClick = () => {
    ...
    // Add this line
    setGeneralSenderEmail("noreply@maubentech.com")
  }
  ...
  const handleSendGeneralEmail = async () => {
    ...
    try {
      ...
      // Add the sender email to the FormData:
      formData.append("senderEmail", generalSenderEmail)
      ...
      // Add attachments
      generalEmailAttachments.forEach((file) => {
        formData.append("attachments", file)
      })
    ...
    <div className="overflow-x-auto">
                  ...
                  <DropdownMenuContent align="end">
                    ...
                    {!isTest && (
                      <DropdownMenuItem
                        ...
                        className={
                          isHidden ? "text-green-600 focus:text-green-600" : "text-purple-600 focus:text-purple-600"
                        }
                      >
                        {isHidden ? (
                          <>
      ...
      {data.length === 0 && (
        <div className="p-8 text-center text-gray-500">
          {isHidden
            ? "No hidden RSVPs found."
            : "No RSVPs yet. The data will appear here once people start submitting the form."}
        </div>
      )}
    </div>
  )
      ...
      <div className="min-h-screen bg-[#F5F1E8] flex items-center justify-center px-4">
        <div className="bg-white rounded-3xl p-8 shadow-xl max-w-md w-full">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            {loginError && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-red-700 text-sm">{loginError}</div>
            )}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="test" className="relative">
                Test RSVPs ({testRsvps.length})
                {testRsvps.length > 0 && <TestTube size={16} className="ml-2 text-orange-500" />}
              </TabsTrigger>
              <TabsTrigger value="hidden" className="relative">
                Hidden RSVPs ({hiddenRsvps.length})
                {hiddenRsvps.length > 0 && <EyeOff size={16} className="ml-2 text-purple-500" />}
              </TabsTrigger>
            </TabsList>
              ...
              <div className="bg-white rounded-lg shadow-lg overflow-hidden">
                <div className="p-6 bg-[#F5F1E8] border-b">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <Button
                      onClick={() =>
                        exportToCSV(rsvps, `maubentech-rsvps-production-${new Date().toISOString().split("T")[0]}.csv`)
                      }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-[#6B8E23]">
                        {rsvps.reduce((sum, r) => sum + (r.guest_count || 0), 0)}
                      </div>
                ...
                <div className="p-6 bg-orange-50 border-b border-orange-200">
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <div className="flex gap-2">
                      <Button
                        onClick={() =>
                          exportToCSV(testRsvps, `maubentech-rsvps-test-${new Date().toISOString().split("T")[0]}.csv`)
                        }
                  ...
                  <div className="bg-orange-100 border border-orange-300 rounded-lg p-4 mb-4">
                    <p className="text-orange-800 text-sm">
                      <strong>⚠️ Test Environment:</strong> This section contains test RSVPs that should be removed
                      before going live. Use the "Cleanup Test Data" button to remove all test entries when ready for
                      production.
                    </p>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {testRsvps.filter((r) => r.attending === "yes").length}
                      </div>
                      <div className="text-[#5D4E37] text-sm">Test Attending</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-amber-600">
                        {testRsvps.filter((r) => r.is_vip).length}
                      </div>
                ...
                <div className="p-6 bg-purple-50 border-b border-purple-200">
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        onClick={() =>
                          exportToCSV(
                            hiddenRsvps,
                            `maubentech-rsvps-hidden-${new Date().toISOString().split("T")[0]}.csv`,
                          )
                        }
                  ...
                  <div className="bg-purple-100 border border-purple-300 rounded-lg p-4 mb-4">
                    <p className="text-purple-800 text-sm">
                      <strong>ℹ️ Hidden RSVPs:</strong> These are production RSVPs that have been hidden from the main
                      production view. They are still valid RSVPs and count towards your event totals, but are kept
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    ...
                    <div className="text-center">
                      <div className="text-2xl font-bold text-green-600">
                        {hiddenRsvps.filter((r) => r.attending === "yes").length}
                      </div>
                      <div className="text-[#5D4E37] text-sm">Hidden Attending</div>
                    </div>
                    <div className="text-center">
                      <div className="text-2xl font-bold text-amber-600">
                        {hiddenRsvps.filter((r) => r.is_vip).length}
                      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <strong className="text-[#2C3E2D]">{rsvpToDelete?.full_name}</strong>? This action cannot be undone.
              <br />
              <br />
              <span className="text-sm text-[#5D4E37]">
                Note: After deletion, this link identifier will be available for new RSVPs.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
          ...
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              ...
              <span className="text-sm text-orange-700 bg-orange-50 p-2 rounded">
                <strong>⚠️ Production Warning:</strong> Only do this when you're ready to go live with the production
                system.
              </span>
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <Dialog open={emailDialogOpen} onOpenChange={setEmailDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            {/* Template Selection */}
            <div>
              <Label className="text-[#2C3E2D] font-semibold mb-4 block">Email Template</Label>
              <RadioGroup value={emailTemplate} onValueChange={setEmailTemplate} className="flex gap-6">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="roots" id="email-template-roots" />
                  <Label htmlFor="email-template-roots">Roots Template</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="maubentech" id="email-template-maubentech" />
                  <Label htmlFor="email-template-maubentech">MaubenTech Template</Label>
                </div>
              </RadioGroup>
            </div>
              ...
              <>
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-1">
                    This message will be sent to {emailRSVP?.full_name} with the selected template branding.
                  </p>
                </div>
              </>
            )}
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 mt-1">
                You can attach multiple files of any type. Maximum total size: 25MB per email.
              </p>
            </div>
          </div>
      ...
      <Dialog open={generalEmailDialogOpen} onOpenChange={setGeneralEmailDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6 py-4">
            ...
            <div>
              <Label className="text-[#2C3E2D] font-semibold mb-4 block">Email Template</Label>
              <RadioGroup value={generalEmailTemplate} onValueChange={setGeneralEmailTemplate} className="flex gap-6">
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="roots" id="general-template-roots" />
                  <Label htmlFor="general-template-roots">Roots Template</Label>
                </div>
                <div className="flex items-center space-x-2">
                  <RadioGroupItem value="maubentech" id="general-template-maubentech" />
                  <Label htmlFor="general-template-maubentech">MaubenTech Template</Label>
                </div>
              </RadioGroup>
            </div>
            ...
            <div>
              <Label className="text-[#2C3E2D] font-semibold mb-4 block">Send From</Label>
              <Select value={generalSenderEmail} onValueChange={setGeneralSenderEmail}>
                <SelectTrigger className="border-[#6B8E23] focus:border-[#B8860B] focus:ring-[#B8860B]">
                  <SelectValue placeholder="Select sender email" />
                </SelectTrigger>
                <SelectContent>
                  <SelectItem value="noreply@maubentech.com">noreply@maubentech.com</SelectItem>
                  <SelectItem value="info@maubentech.com">info@maubentech.com</SelectItem>
                  <SelectItem value="onyecherebenedict@maubentech.com">onyecherebenedict@maubentech.com</SelectItem>
                  <SelectItem value="anijimjamesmaurice@maubentech.com">anijimjamesmaurice@maubentech.com</SelectItem>
                  <SelectItem value="noahvikoo@maubentech.com">noahvikoo@maubentech.com</SelectItem>
                </SelectContent>
              </Select>
              <p className="text-sm text-gray-500 mt-1">Choose which email address to send from</p>
            </div>
              ...
              <div className="space-y-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {linkIdentifierMode === "new" ? (
                    <div>
                      ...
                      <p className="text-sm text-gray-500 mt-1">
                        This will be used to create the unique RSVP link for the invitee
                      </p>
                    </div>
                  ) : (
                    <div>
                      ...
                      <Select
                        ...
                        onValueChange={setSelectedExistingLinkId}
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500 mt-2">
                    Hidden RSVPs are still production RSVPs but won't appear in the main production list. Use this for
                    internal invitations or special categories.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-6 py-4">
            ...
            <div className="space-y-4 p-4 bg-blue-50 rounded-lg border border-blue-200">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {whatsappLinkMode === "new" ? (
                  <div>
                    ...
                    />
                    {isCheckingWhatsappLinkIdentifier && (
                      <p className="text-sm text-blue-600 mt-1">Checking availability...</p>
                    )}
                    {whatsappLinkIdentifierError && (
                      <p className="text-sm text-red-600 mt-1">{whatsappLinkIdentifierError}</p>
                    )}
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500 mt-1">
                      Choose from existing link identifiers that haven't been used yet
                    </p>
                  </div>
                )}
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mt-2">
                  Hidden RSVPs won't appear in the main production list but are still valid RSVPs.
                </p>
              </div>
            </div>
