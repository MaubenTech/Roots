...
db.serialize(() => {
  ...
  `,
	...
	// Create index for is_hidden column in rsvps table
	db.run("CREATE INDEX IF NOT EXISTS idx_rsvps_hidden ON rsvps(is_hidden)", (err) => {
		if (err) {
			console.error("Error creating hidden index:", err);
		} else {
			console.log("Hidden index created");
		}
	});
