...
// Function to run SQL commands sequentially
function runSequential(commands, callback) {
	...
	db.run(command.sql, command.params || [], (err) => {
		if (err) {
			console.error(`Error executing: ${command.description}`, err);
		} else {
			console.log(`✓ ${command.description}`);
		}
		runSequential(commands, callback);
	});
}

db.serialize(() => {
	// Step 1: Create rsvps table first (since link_identifiers references it)
	db.run(
		`
  ...
  `,
		(err) => {
			...
			createLinkIdentifiersTable();
		}
	);

	function createLinkIdentifiersTable() {
		// Step 2: Check if link_identifiers table exists and get its structure
		db.all("PRAGMA table_info(link_identifiers)", (err, columns) => {
			if (err) {
				console.error("Error checking link_identifiers table structure:", err);
				return;
			}
			...
			if (!tableExists) {
				// Create table with tracking_number column
				console.log("Creating link_identifiers table with tracking_number...");
				db.run(
					`
          CREATE TABLE link_identifiers (
            id INTEGER PRIMARY KEY AUTOINCREMENT,
        ...
        `,
					(err) => {
						...
						console.log('✓ Table "link_identifiers" created successfully with tracking_number');
						checkRsvpsTable();
					}
				);
			} else if (!hasTrackingNumber) {
				// Add tracking_number column to existing table
				console.log("Adding tracking_number column to existing link_identifiers table...");
				db.run(`ALTER TABLE link_identifiers ADD COLUMN tracking_number INTEGER`, (err) => {
					if (err) {
						console.error("Error adding tracking_number column:", err);
						return;
					}
					console.log('✓ Column "tracking_number" added to link_identifiers table');
					checkRsvpsTable();
				});
			} else {
				console.log('✓ Table "link_identifiers" already exists with tracking_number column');
				checkRsvpsTable();
			}
		});
	}

	function checkRsvpsTable() {
		// Step 3: Check if link_identifier_id column exists in rsvps table
		db.all("PRAGMA table_info(rsvps)", (err, columns) => {
			if (err) {
				console.error("Error checking rsvps table structure:", err);
				return;
			}
			...
			if (!hasLinkIdentifierColumn) {
				console.log("Adding link_identifier_id column to rsvps table...");
	...
	function createIndexesAndPopulate() {
		console.log("Creating indexes...");

		const indexCommands = [
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_rsvps_email ON rsvps(email)",
				description: "RSVP email index created",
			},
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_rsvps_attending ON rsvps(attending)",
				description: "RSVP attending index created",
			},
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_link_identifiers_uuid ON link_identifiers(uuid)",
				description: "UUID index created",
			},
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_link_identifiers_vip ON link_identifiers(is_vip)",
				description: "VIP index created",
			},
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_link_identifiers_tracking ON link_identifiers(tracking_number)",
				description: "Tracking number index created",
			},
			{
				sql: "CREATE INDEX IF NOT EXISTS idx_rsvps_link_identifier ON rsvps(link_identifier_id)",
				description: "Link identifier index created",
			},
		];

		runSequential(indexCommands, () => {
			console.log("All indexes created successfully!");

			// Step 5: Check if we need to populate the table
			db.get("SELECT COUNT(*) as count FROM link_identifiers WHERE uuid NOT LIKE 'test-%'", (err, row) => {
				...
				if (row.count === 0) {
					console.log("Populating link_identifiers table...");
					populateTable();
				} else {
					console.log(`✓ Link identifiers table already has ${row.count} production entries`);
					closeDatabase();
				}
			});
		});
	...
	function populateTable() {
		...
		function checkCompletion() {
			...
			if (insertCount === totalInserts) {
				console.log(`\n🎉 Migration completed! Inserted ${totalInserts} link identifiers.`);
		...
		productionUuids.forEach((uuid, index) => {
			...
			db.run("INSERT INTO link_identifiers (tracking_number, uuid, is_vip) VALUES (?, ?, ?)", [trackingNumber, uuid, isVip], (err) => {
				if (err) {
	...
	function closeDatabase() {
		db.close((err) => {
			if (err) {
				...
				console.log("\nYour RSVP system is now ready to use with production UUID-based invitation links.");
				console.log("\nExample links:");
				console.log("VIP #1: yourdomain.com/fbf0ae67-c05a-482f-b6ea-5aafb45df8fa");
				console.log("Regular #21: yourdomain.com/e59bb81b-aac3-4f6a-baae-5735afe9c555");
				console.log("Test VIP: yourdomain.com/test-vip-001-fbf0ae67-c05a-482f-b6ea-5aafb45df8fa");
			}
		});
	}
});
