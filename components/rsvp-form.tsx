					...
					{showExistingRSVP && existingRSVP ? (
						<div className="text-center">
							...
							<p className="text-lg text-[#5D4E37] mb-8">
								Thank you, <span className="font-semibold text-[#6B8E23]">{existingRSVP.full_name}</span>! We've already received your RSVP for
								the Corporate Cocktail & Fundraiser Evening.
							</p>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-left">
									...
									<div className="flex items-center gap-3">
										...
										<div>
											...
											<p className="font-medium text-[#2C3E2D]">{new Date(existingRSVP.created_at).toLocaleDateString()}</p>
										</div>
									</div>
							...
							<div className="space-y-4">
								<p className="text-[#5D4E37]">Need to make changes to your RSVP? Click the button below to update your response.</p>
								<Button onClick={handleUpdateRSVP} className="bg-[#6B8E23] hover:bg-[#5A7A1F] text-white px-8 py-3 rounded-full font-semibold">
									Update My RSVP
								</Button>
							</div>
						...
						<form onSubmit={handleSubmit}>
										...
										{isVip && (
											<>
												<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
													<Label className="text-[#2C3E2D] font-semibold mb-4 block">Will you be attending with a guest?</Label>
													<RadioGroup
							...
							<motion.div
								...
								<Button
									type="submit"
									disabled={isSubmitting || !formData.fullName || !formData.email || !formData.phone || !formData.attending}
